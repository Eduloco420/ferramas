...
class OrchestratorController:
    ...
    def crear_producto(self):
        ...
        if not all([nombre, marca, codigo, precio, images is not None]):
        ...
        try:
            ...
            return jsonify({
                    'error': 'Error al comunicar con microservicios', 
                    'detalle': str(e)
                }), 500
    ...
    def obtener_venta(self, id):
        try:
            response = requests.get(f'{URL_MS_VENTAS}/venta/{id}')
            print(response)
            ...
            return make_response(jsonify({
                'mensaje': 'Error conectando al servicio de ventas',
                'error': str(e)
            }), 500)
    ...
    def ver_imagen(self,filename):
        try:
            response = requests.get(f'{URL_MS_IMG}/img/archivo/{filename}')
            if response.status_code != 200:
